...
if isempty(hbase)
   ...
   tf = all(tf);
   if isa(hbase,'matlab.graphics.chart.primitive.BoxChart')
       tf = 1;
   end
end
...
if ishghandle(A,'patch') || ishghandle(A,'Bar') || ishghandle(A,'area') ...
   ...
   pname = 'EdgeColor';
elseif isa(A,'matlab.graphics.chart.primitive.BoxChart')
   pname = 'BoxLineColor';
...
elseif isa(A,'matlab.graphics.chart.primitive.BoxChart') %HG2: Quadrateral underlying data object
   [V,F] = getQuadrilateralData(A.NodeChildren(5));
end

end

function [V,F] = getQuadrilateralData(A) % surface, bar, histogram, boxchart
